...
import ids.clustering.algorithm.HMRFWKmeansMMD;
import ids.clustering.model.Domain;
import ids.utils.ConstraintsUtils;

public class ConstraintWBasedMMD {
	...
	public ConstraintWBasedMMD(Domain d1, Domain d2, ExperimentParams par) {
		this.d1 = d1;
		this.d2 = d2;
		
		parseParams(par);
		if (verbose) {
			System.out.println("Constrained Weight Based with MMD");
			System.out.println("Input parameters:");
			System.out.println("1. verbose = " + this.verbose);
			System.out.println("2. debug = " + this.debug);
			System.out.println("3. use transitive closure = " + this.useTC);
			System.out.println("4. inter cannot link constraints = " + this.inferCannotLink);
		}
	...
	public void Start() {
		...
		par1.centeroids = d1.centroids; // start algorithm from this seed
		...
		while (t < maximum_number_iterations) {
			...
			HMRFWKmeansMMD d1_kmeans = new HMRFWKmeansMMD(d1.data, d1.k, par1);
			d1_kmeans.cluster();
			d1.old_idx = d1.idx;
			...
			HMRFWKmeansMMD d2_kmeans = new HMRFWKmeansMMD(d2.data, d2.k, par2);
			d2_kmeans.cluster();
			d2.old_idx = d2.idx;
